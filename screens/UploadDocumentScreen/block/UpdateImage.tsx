...
import {DocsIcon, UploadIcon1} from '../../../assets/icons';
      ...
      <View style={[tailwind('mb-3')]}>
        ...
        {props?.fileResponse?.length != 0 ? (
          <View style={[tailwind('flex-row ')]}>
            {/* {props?.fileResponse?.length != 0 && props?.fileResponse.map(item => {
            return (
              <View key={item?.uri} style={[tailwind('relative ml-3')]}>
                <TouchableOpacity onPress={()=>props?.removeDoc(item?.uri)}
                  style={[
                    tailwind('absolute'),
                ...
                <View>
                  <Text numberOfLines={2} style={[tailwind('text-center'),{width:50,}]}>
                  {item?.name}
                  </Text>
                </View>
              </View>
